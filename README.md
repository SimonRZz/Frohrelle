...
in a hotel room, and the station does not have to be in the same building. Over
an internet link it can stay at home while you are somewhere else entirely. What
the phone does not give you is a paddle. SSB and digital modes are fine on a
touchscreen, CW is not, and keyboard style sending on a phone is no substitute
for a paddle you have used for years. This box gives the phone a real paddle
input.
...
The whole operating position then fits in a jacket pocket: phone, interface,
paddle, one short cable. The radio can be a few hundred kilometres away.
...
```
  ...
  +--------+     +--------------+               +------------+         +-----------+
  |  DIT   |-----| GPIO4        |   USB MIDI    |  SmartSDR  |  LAN or |  keyer,   |
  |  DAH   |-----| GPIO5  ESP32 |-------------->|            |-------->|  sidetone |
  |  GND   |-----| GND      -S3 |  note 20/21   |            | internet|  TX       |
  +--------+     +--------------+               +------------+         +-----------+
```

The link between the app and the radio can be the local network or a remote
connection over the internet. The interface neither knows nor cares: it hands
its notes to the app, and the app deals with the rest. Element lengths are
generated by the keyer in the radio, so network jitter cannot stretch or shrink
what actually goes on the air. What you do notice on a remote link is the round
trip delay in the sidetone coming back to you.
...
| Sidetone lags behind the paddle on a remote link | Network round trip, nothing the interface can influence. The transmitted elements are still formed in the radio |
...
Working version, in daily use with SmartSDR on an iPhone, locally and remotely. The firmware
